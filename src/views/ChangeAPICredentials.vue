		...
		<div class="loader loader-danger">
			<div class="loader-content">
				...
				<transition name="fade" mode="out-in" appear>
					<div class="loader-text">Please verify your Sia API password</div>
				</transition>
				<transition name="fade" mode="out-in" appear>
					<div>
						<div class="control">
						...
						<button class="btn btn-inline btn-success" @click="onRetryPassword" :disabled="waiting">Try Again</button>
					</div>
				</transition>
			</div>
		</div>
...
export default {
	...
	async mounted() {
		...
		try {
			...
			this.password = await client.getDefaultAPIPassword();

			window.setTimeout(() => {
				this.$emit('animated');
			}, 6000);
		} catch (ex) {
			console.error('ChangeAPICredentials.mounted', ex.message);
		} finally {
			this.waiting = false;
		}
	},
	methods: {
		async onRetryPassword() {
			if (this.waiting)
				return;
			...
			try {
				...
				const client = new SiaAPIClient({
					...this.config,
					siad_api_password: this.password
				});
				...
				log.error('ChangeAPICredentials.onRetryPassword', ex.message);
